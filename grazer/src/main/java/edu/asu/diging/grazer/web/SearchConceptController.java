...
public class SearchConceptController {
    ...
    public ResponseEntity<String> searchConcepts(@RequestParam("term") String term) {
        
        ConceptpowerConcepts cpc = connector.search(term);
        List<ConceptpowerConcept> conceptList = cpc.getConceptEntries();

        cpc = connector.search(term);
        conceptList.addAll(cpc.getConceptEntries());
